...
export default function FilterButtons({ onChange }) {
  ...
  return (
    <>
      <aside className="flex gap-2 md:gap-4">
        {BUTTON_LABELS.map((label) => (
          <button
            key={label}
            onClick={() => handleMaker(label)}
            value={label}
            className={`bg-light px-4 py-2 rounded-full text-base md:text-lg text-primary hover:bg-primary hover:text-white hover:-translate-y-1 transition cursor-pointer ${
              selectedMaker === label
